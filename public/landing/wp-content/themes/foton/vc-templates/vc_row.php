...
if ( $row_content_width === 'grid' ) {
	$mkdf_row_wrapper_start .= '<div class="mkdf-row-grid-section-wrapper ' . esc_attr( $grid_row_classes ) . '" ' . esc_attr( $grid_row_datas ) . ' ' . foton_mikado_get_inline_style( $mkdf_grid_row_styles ) . '><div class="mkdf-row-grid-section">';
	$mkdf_row_wrapper_end   .= '</div></div>';
}

$mkdf_row_data = array();
if (!empty($dynamic_background_color_value)) {
	$mkdf_row_data['data-dynamic-bgrnd'] = $dynamic_background_color_value;
}
...
$output .= '<div ' . implode( ' ', $wrapper_attributes ) . ' ' . foton_mikado_get_inline_style( $mkdf_vc_row_styles ) .' ' . foton_mikado_get_inline_attrs($mkdf_row_data) . '>';
