...
/*
   Class: FotonMikadoClassTaxonomyField
   A class that initializes FotonMikadoClass Taxonomy Field
*/

class FotonMikadoClassTaxonomyField implements iFotonMikadoInterfaceRender {
	private $type;
	private $name;
	private $label;
	private $description;
	...
	function __construct( $type, $name, $label = "", $description = "", $options = array(), $args = array(), $hidden_property = "", $hidden_values = array() ) {
		...
		add_filter( 'foton_mikado_filter_taxonomy_fields', array( $this, 'addFieldForEditSave' ) );
	}
...
abstract class FotonMikadoClassTaxonomyFieldType {
	abstract public function render( $name, $label = "", $description = "", $options = array(), $args = array() );
}

class FotonMikadoClassTaxonomyFieldText extends FotonMikadoClassTaxonomyFieldType {
...
class FotonMikadoClassTaxonomyFieldImage extends FotonMikadoClassTaxonomyFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $hidden = false) {
		if ( ! isset( $_GET['tag_ID'] ) ) { ?>
			<div class="form-field">
				<label for="<?php echo esc_html( $name ); ?>"><?php echo esc_html( $label ); ?></label>
				...
				<p>
					<input type="button" class="button button-secondary mkdf-tax-media-add" name="mkdf-tax-media-add" value="<?php esc_attr_e( 'Add Image', 'foton' ); ?>"/>
					<input type="button" class="button button-secondary mkdf-tax-media-remove" name="mkdf-tax-media-remove" value="<?php esc_attr_e( 'Remove Image', 'foton' ); ?>"/>
				</p>
			</div>
			<?php
		} else {
			...
			<tr class="form-field" <?php if ($hidden) { ?> style="display: none"<?php } ?>>
				...
				<td>
					...
					<p>
						<input type="button" class="button button-secondary mkdf-tax-media-add" name="mkdf-tax-media-add" value="<?php esc_attr_e( 'Add Image', 'foton' ); ?>"/>
						<input data-termid="<?php echo esc_attr( $_GET['tag_ID'] ); ?>" data-taxonomy="<?php echo esc_attr( $taxonomy ); ?>" type="button" class="button button-secondary mkdf-tax-media-remove" name="mkdf-tax-media-remove" value="<?php esc_attr_e( 'Remove Image', 'foton' ); ?>"/>
					</p>
				</td>
			</tr>
			<?php
...
class FotonMikadoClassTaxonomyFieldSelect extends FotonMikadoClassTaxonomyFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $hidden = false ) {
...
class FotonMikadoClassTaxonomyFieldIcon extends FotonMikadoClassTaxonomyFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $hidden = false ) {
		...
		if ( ! isset( $_GET['tag_ID'] ) ) { ?>
			...
			<?php foreach ( $icons_collections as $icons_collection ) {
				...
				<div class="form-field mkd-icon-collection-holder" style="display: none" data-icon-collection="<?php echo esc_attr( $icons_collection ); ?>">
					<label for="<?php echo esc_attr( $name ) . '_icon'; ?>"><?php esc_html_e( 'Icon', 'foton' ); ?></label>
			...
			<?php foreach ( $icons_collections as $icons_collection ) {
				...
				<tr class="form-field mkd-icon-collection-holder" style="<?php echo esc_attr( $style ); ?>" data-icon-collection="<?php echo esc_attr( $icons_collection ); ?>">
					<th scope="row"><?php esc_html_e( 'Icon', 'foton' ); ?></th>
					<td>
						<select name="<?php echo esc_attr( $name . '_' . $icons_param ) ?>" id="<?php echo esc_attr( $name . '_' . $icons_param ) ?>">
							<?php
							$icons      = foton_mikado_icon_collections()->getIconCollection( $icons_collection );
...
class FotonMikadoClassTaxonomyFieldColor extends FotonMikadoClassTaxonomyFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $hidden = false ) {
...
class FotonMikadoClassTaxonomyFieldFactory {
	public function render( $field_type, $name, $label = "", $description = "", $options = array(), $args = array(), $hidden = false ) {
		
		switch ( strtolower( $field_type ) ) {
			case 'text':
				$field = new FotonMikadoClassTaxonomyFieldText();
				$field->render( $name, $label, $description, $options, $args, $hidden );
				break;
			
			case 'image':
				$field = new FotonMikadoClassTaxonomyFieldImage();
				$field->render( $name, $label, $description, $options, $args, $hidden );
				break;
			
			case 'selectblank':
				$field = new FotonMikadoClassTaxonomyFieldSelect();
				$field->render( $name, $label, $description, $options, $args, $hidden );
				break;
			
			case 'icon':
				$field = new FotonMikadoClassTaxonomyFieldIcon();
				$field->render( $name, $label, $description, $options, $args, $hidden );
				break;
			
			case 'color':
				$field = new FotonMikadoClassTaxonomyFieldColor();
				$field->render( $name, $label, $description, $options, $args, $hidden );
				break;
