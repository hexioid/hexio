...
class FotonMikadoClassSocialIconWidget extends FotonMikadoClassWidget {
	
	public function __construct() {
		parent::__construct(
			'mkdf_social_icon_widget',
			esc_html__( 'Foton Social Icon Widget', 'foton' ),
			array( 'description' => esc_html__( 'Add social network icons to widget areas', 'foton' ) )
		);
		...
		$this->params = array_merge(
			array(
				array(
					'type'  	  => 'dropdown',
					'name'  	  => 'icon_text',
					'title' 	  => esc_html__( 'Display Icon or Text', 'foton' ),
					'description' => esc_html__( 'Choose whether you want to display social icon or text instead', 'foton' ),
					'options' 	  => array(
						'icon' => esc_html__( 'Icon', 'foton' ),
						'text' => esc_html__( 'Text', 'foton' )
					)
				),
				array(
					'type'        => 'textfield',
					'name'        => 'social_text',
					'title'       => esc_html__( 'Social Text', 'foton' ),
					'description' => esc_html__( 'Instert text which will describe desired social network', 'foton' )
				)
			),
			foton_mikado_icon_collections()->getSocialIconWidgetParamsArray(),
			array(
				array(
					'type'  => 'textfield',
					'name'  => 'link',
	...
	public function widget( $args, $instance ) {
		...
		?>
		<a class="mkdf-social-icon-widget-holder mkdf-icon-has-hover" <?php echo foton_mikado_get_inline_attr( $hover_color, 'data-hover-color' ); ?> <?php foton_mikado_inline_style( $icon_styles ) ?> href="<?php echo esc_url( $link ); ?>" target="<?php echo esc_attr( $target ); ?>">
