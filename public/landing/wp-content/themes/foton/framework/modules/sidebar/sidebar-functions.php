...
if ( ! function_exists( 'foton_mikado_get_sidebar_holder_class' ) ) {
	/**
	 * Return classes for sidebar holder when sidebar is active
	 *
	 * @return string
	 */
	function foton_mikado_get_sidebar_holder_class() {
		$sidebar_layout = foton_mikado_sidebar_layout();
		...
		return foton_mikado_get_class_attribute( $sidebar_class );
	}
}

if ( ! function_exists( 'foton_mikado_get_sidebar' ) ) {
	/**
	 * Return Sidebar name
	 *
	 * @return string
	 */
	function foton_mikado_get_sidebar() {
		$sidebar_name                = 'sidebar';
		$custom_sidebar_area         = foton_mikado_get_meta_field_intersect( 'custom_sidebar_area' );
		$custom_archive_sidebar_area = foton_mikado_options()->getOptionValue( 'archive_custom_sidebar_area' );
		$custom_search_sidebar_area  = foton_mikado_options()->getOptionValue( 'search_custom_sidebar_area' );
		$custom_single_sidebar_area  = foton_mikado_get_meta_field_intersect( 'blog_single_custom_sidebar_area' );
		...
		if ( ( is_archive() || ( is_home() && is_front_page() ) ) && ! foton_mikado_is_woocommerce_page() && ! empty( $custom_archive_sidebar_area ) ) {
			$sidebar_name = $custom_archive_sidebar_area;
		}
		
		return apply_filters( 'foton_mikado_filter_sidebar_name', $sidebar_name );
	}
}

if ( ! function_exists( 'foton_mikado_get_custom_sidebars' ) ) {
	/**
	 ...
	 */
	function foton_mikado_get_custom_sidebars() {
		$foton_custom_sidebars = get_option( 'mkdf_sidebars' );
		$formatted_array             = array();
		
		if ( is_array( $foton_custom_sidebars ) && count( $foton_custom_sidebars ) ) {
			foreach ( $foton_custom_sidebars as $custom_sidebar ) {
...
if ( ! function_exists( 'foton_mikado_get_custom_sidebars_options' ) ) {
	function foton_mikado_get_custom_sidebars_options( $enable_default = false ) {
		$sidebar_options = array();
		
		if ( $enable_default ) {
			$sidebar_options[''] = esc_html__( 'Default', 'foton' );
		}
		
		$sidebar_options['no-sidebar']       = esc_html__( 'No Sidebar', 'foton' );
		$sidebar_options['sidebar-33-right'] = esc_html__( 'Sidebar 1/3 Right', 'foton' );
		$sidebar_options['sidebar-25-right'] = esc_html__( 'Sidebar 1/4 Right', 'foton' );
		$sidebar_options['sidebar-20-right'] = esc_html__( 'Sidebar 1/5 Right', 'foton' );
		$sidebar_options['sidebar-33-left']  = esc_html__( 'Sidebar 1/3 Left', 'foton' );
		$sidebar_options['sidebar-25-left']  = esc_html__( 'Sidebar 1/4 Left', 'foton' );
		$sidebar_options['sidebar-20-left']  = esc_html__( 'Sidebar 1/5 Left', 'foton' );
