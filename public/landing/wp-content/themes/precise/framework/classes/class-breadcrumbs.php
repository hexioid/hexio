...
class Precise_Breadcrumbs {
	...
	function __construct( $args = array() ) {
		...
		$defaults = array(
			...
			'labels'            => array(),
			'date_labels'       => array(),
			'action'            => 'precise/action/breadcrumbs/render_html',
			'css_namespace'     => array(),
			'path_type'         => 'full', // minified , full
			'post_taxonomy'     => apply_filters(
				'precise/filter/breadcrumbs/trail_taxonomies',
				array(
					'post'      => 'category',
					'product'	=> 'product_cat'
				)
		...
		$this->args = apply_filters( 'precise/filter/breadcrumbs/args', wp_parse_args( $args, $defaults ) );
	...
	public function get_trail() {
		...
		$wrapper_classes = apply_filters( 'precise/filter/breadcrumbs/wrapper_classes', $wrapper_classes );
	...
	public function get_items() {
		...
		$breadcrumb = apply_filters( 'precise/filter/breadcrumbs/trail', $breadcrumb, $this->args );
	...
	public function default_labels() {
		...
		$labels = array(
			'browse'              => esc_html_x( 'Browse:', 'front-view', 'precise' ),
			'home'                => $this->home_title(),
			'error_404'           => esc_html_x( '404 Not Found', 'front-view', 'precise' ),
			'archives'            => esc_html_x( 'Archives', 'front-view', 'precise' ),
			'search'              => esc_html_x( 'Search results for &#8220;%s&#8221;', 'front-view', 'precise' ),
			'paged'               => esc_html_x( 'Page %s', 'front-view', 'precise' ),
			'archive_minute'      => esc_html_x( 'Minute %s', 'front-view', 'precise' ),
			'archive_week'        => esc_html_x( 'Week %s', 'front-view', 'precise' ),
	...
	public function home_title() {
		...
		$title            = esc_html_x( 'Home', 'front-view', 'precise' );
		$use_custom_title = apply_filters( 'precise/filter/breadcrumbs/custom_home_title', false );
	...
	public function build_trail() {
		...
		$custom_trail = apply_filters( 'precise/filter/breadcrumbs/custom_trail', false, $this->args );
		 ...
		 */
		$this->items = apply_filters( 'precise/filter/breadcrumbs/items', $this->items, $this->args, $this );
	...
	public function add_post_terms( $post_id ) {
		...
		if ( ! empty( $this->args['post_taxonomy'][ $post_type ] ) ) {
			add_filter('get_object_terms', 'precise_exclude_demo_term_in_category');
			$post_terms = wp_get_post_terms( $post_id, $this->args['post_taxonomy'][ $post_type ] );
			remove_filter('get_object_terms', 'precise_exclude_demo_term_in_category');
	...
	public function prepare_label( $label, $fallback = false ) {
		...
		if ( false !== $fallback ) {
			$fallback = sprintf(
				apply_filters( 'precise/filter/breadcrumbs/icon_fallback', '<span class="hidden">%s</span>', $fallback ),
				esc_attr( $fallback )
			);
		}
		...
		// Return default icon
		return sprintf(
			apply_filters( 'precise/filter/breadcrumbs/default_icon_format', '<span class="%1$s">%2$s</span>', $label, $fallback ),
