...
class Precise_Widget_Price_Filter_List extends WP_Widget {
	...
	public function __construct() {
		$widget_ops  = array(
			...
			'description' => esc_html__( 'Shows a price filter list in a widget which lets you narrow down the list of shown products when viewing product categories.', 'precise' ),
		);
		$control_ops = array( 'width' => 400, 'height' => 350 );
		parent::__construct( 'la-price-filter-list', esc_html__( '[Precise] - Price Filter List', 'precise' ), $widget_ops, $control_ops );
	}
			...
			if ( $price_list ) {
				...
				foreach ( $price_list as $price ) {
					...
					if ( $price_attr ) {
						...
						$link = Precise_Helper::get_base_shop_url();
						if ( $min_price == $min_price_actived ) {
							if ( $params ) {
								$link .= '?' . $params;
							}
	...
	public function form( $instance ) {
		...
		?>
		<p><label for="<?php echo esc_attr($this->get_field_id( 'title' )); ?>"><?php esc_html_e( 'Title:', 'precise' ); ?></label>
			<input class="widefat" id="<?php echo esc_attr($this->get_field_id( 'title' )); ?>" name="<?php echo esc_attr($this->get_field_name( 'title' )); ?>" type="text" value="<?php echo esc_attr( $title ); ?>" />
		</p>

		<p><label for="<?php echo esc_attr($this->get_field_id( 'text' )); ?>"><?php esc_html_e( 'Content:', 'precise' ); ?></label>
		...
		<p><?php esc_html_e( 'Enter the price by format: min_price|max_price. Divide price list with linebreaks (Enter). Example:', 'precise' ); ?></p>
		<p><?php esc_html_e( '100|200', 'precise' ); ?><br><?php esc_html_e( '200|300', 'precise' ); ?>
			<br><?php esc_html_e( '300|400', 'precise' ); ?></p>
		<?php
	}
}
