...
/**
 * The Autoloader class for Precise.
 */
class Precise_Autoload {
    ...
    public function __construct( $args = array() ) {
        ...
        if ( empty( self::$transient_name ) ) {
            self::$transient_name = 'precise_autoloader_paths_' . md5( __FILE__ );
        }
    ...
    protected function get_path( $class_name ) {
        ...
        $paths = array();
        if ( 0 === stripos( $class_name, 'Precise' ) ) {
            $path     = wp_normalize_path( Precise::$template_dir_path . '/framework/classes/' );

            $class_name = str_replace( array( 'Precise_' ), '', $class_name );
