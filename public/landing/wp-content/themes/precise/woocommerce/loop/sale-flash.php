...
if( $product->is_on_sale() ){
	...
	$message = esc_html_x( 'Sale!', 'front-view', 'precise' );
	...
	if ( $product->has_child() ) {
		...
		$text = apply_filters('precise/filter/product/sale_badge_title', esc_html_x( 'Save up to', 'front-view', 'precise' ), $product);
		...
		$text = apply_filters('precise/filter/product/sale_badge_title', esc_html_x( 'Save', 'front-view', 'precise' ), $product);
	...
	if( $save_percentage > 0 && $saving_price > 0 ) {
		...
		$saving_price_r = apply_filters('precise/filter/product/sale_badge_price', sprintf( _x( ' %s', 'front-view', 'precise' ), $saving_price ), $saving_price, $product);
		$message .= sprintf('<span class="save-total"><span class="hidden">%s</span>%s</span>', $text, $saving_price_r );
	}
	if( $min_price > 0 ){
		$message .= sprintf('<span class="save-sale-price"><span class="hidden">%s</span>%s</span>', esc_html_x( 'Only', 'front-view', 'precise' ), wc_price( $min_price ) );
	}
