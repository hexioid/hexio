  ...
  public function output() {
    ...
    if( isset( $this->field['options'] ) ) {
      ...
      foreach ( $options as $key => $value ) {
        echo '<label><input type="'. $input_type .'" name="'. $this->element_name( $input_attr ) .'" value="'. $key .'"'. $this->element_class() . $this->element_attributes( $key ) . $this->checked( $this->element_value(), $key ) .'/><img src="'. $value .'" alt="'. $key .'" /><i class="la-tab-icon fa fa-check"></i></label>';
      }
    }
