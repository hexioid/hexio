...
if(!class_exists('LaStudio_Swatch')){
    ...
    class LaStudio_Swatch{
        ...
        private function __construct(){
            ...
            $this->ajax_nonce_string = 'la-swatch_ajax';
            ...
            if(is_admin()){
                ...
                add_action( 'wp_ajax_la_swatch_admin_load_thumbnails', array($this, 'admin_load_variation_gallery') );
                add_action( 'woocommerce_save_product_variation', array( $this, 'save_gallery_for_product_variation' ), 10, 2 );
            }
            ...
            add_action( 'wp_ajax_la_swatch_get_product_variations', array($this, 'get_product_variations') );
            add_action( 'wp_ajax_nopriv_la_swatch_get_product_variations', array($this, 'get_product_variations') );
            ...
            add_filter( 'woocommerce_available_variation', array( $this, 'add_additional_into_variation_json' ), 10, 3 );

            add_filter( 'woocommerce_dropdown_variation_attribute_options_html', array( $this, 'override_output_dropdown_variation_attribute_options' ), 101, 3 );
        ...
        public function on_init(){
            ...
            add_image_size( 'la_swatches_image_size', $image_size['width'], $image_size['height'], $image_crop );
        }

        public function admin_menu(){
            add_submenu_page(
                'edit.php?post_type=product',
                esc_html__('Swatches', 'la-studio'),
                esc_html__('Swatches', 'la-studio'),
                'manage_options',
                'la-swatches-page',
                        ...
                        <div class="la-sections">
                            <div class="la-section" style="display:block;">
                                <div class="la-element la-field-spacing">
                                    <div class="la-title">
                                        <h4><?php esc_html_e('Swatches and Photos', 'la-studio' )?></h4>
                                    </div>
                                    <div class="la-fieldset">
                                        <div class="la-parent-fields">
                                            <fieldset>
                                                ...
                                                <div class="la-pseudo-field">
                                                    <label><input name="la_swatches_configs[image_size][crop]" type="checkbox" value="1" <?php checked( $image_crop, 1 ) ?>><?php esc_html_e('Hard crop?','la-studio') ?></label>
                                                </div>
                                            </fieldset>
                                        </div>
                                        <p class="la-text-desc"><?php esc_html_e('The default size for color swatches and photos.', 'la-studio') ?></p>
                                    </div>
                                    <div class="clear"></div>
                                </div>
                                <?php
                                    ...
                                    if(! empty($attribute_array) ) {
                                        ...
                                        echo la_fw_add_element(array(
                                            ...
                                            'type'       => 'checkbox',
                                            'title'      => esc_html__('Swatch Attributes Visibility', 'la-studio'),
                                            'desc'       => esc_html__('Swatch attributes visibility on items from the product listing page', 'la-studio'),
                                        ...
                                        echo la_fw_add_element(array(
                                            'type'    => 'notice',
                                            'class'   => 'warning',
                                            'content' => sprintf(__('Please %s add new attribute %s before to do this!','la-studio'), '<a href="'.admin_url('edit.php?post_type=product&page=product_attributes').'">','</a>'),
                                        ));
                                    }
                                ?>
                            </div>
        ...
        public function admin_enqueue(){
            wp_enqueue_script( 'la-swatches', LaStudio_Plugin::$plugin_dir_url . 'assets/js/swatches-admin.js', array('jquery'), null, true );
            $vars = array(
                'ajax_url' => admin_url( 'admin-ajax.php', 'relative' ),
                'nonce' => wp_create_nonce( $this->ajax_nonce_string )
            );
            wp_localize_script( 'la-swatches', 'la_swatches_vars', $vars );
        }
                ...
                if(isset($data['type'])){
                    ...
                    if($type == 'color'){
                        if(isset($data['color'])){
                            ...
                            $output .= '<span class="type-none">'.esc_html__('Not Set', 'la-studio').'</span>';
                        }
                    }elseif($type == 'photo'){
        ...
        public function product_write_panel_tabs() {
            ?>
            <li class="tab_la_swatches show_if_variable"><a href="#panel_la_swatches"><span><?php esc_html_e('Swatches', 'la-studio') ?></span></a></li>
            <?php
        }
        ...
        public function render_product_tab_content( ) {
            ...
            $default_type = array(
                ...
                'radio'             => esc_html__('Radio Button', 'la-studio')
            );
            ...
            $default_style = array(
                'default' => esc_html__('Default', 'la-studio'),
                'circle' => esc_html__('Circle', 'la-studio'),
                'square' => esc_html__('Square', 'la-studio'),
                'rounder' => esc_html__('Rounder', 'la-studio')
            );

            $default_sub_type = array(
                'color'     => esc_html__('Color', 'la-studio'),
                'photo'     => esc_html__('Image', 'la-studio')
            );
                        ...
                        <table class="widefat">
                            <thead>
                                <th class="attribute_swatch_label">
                                    <?php esc_html_e( 'Product Attribute Name', 'la-studio' ); ?>
                                </th>
                                <th class="attribute_swatch_type">
                                    <?php esc_html_e( 'Attribute Control Type', 'la-studio' ); ?>
                                </th>
                            </thead>
                        </table>
                    </div>
                                    ...
                                    <div class="la_swatch_field_form_mask">
                                        <div class="field_form">
                                            ...
                                            echo la_fw_add_element(array(
                                                ...
                                                'title'          => esc_html__('Swatches and Photos Size', 'la-studio'),
                                                'default'        => 'la_swatches_image_size',
                                                'options'        => array(
                                                    'thumbnail'                 => esc_html__('Thumbnail', 'la-studio'),
                                                    'medium'                    => esc_html__('Medium', 'la-studio'),
                                                    'shop_thumbnail'            => esc_html__('Product thumbnails', 'la-studio'),
                                                    'shop_catalog'              => esc_html__('Catalog images', 'la-studio'),
                                                    'la_swatches_image_size'    => esc_html__('Swatches Image Size', 'la-studio')
                                                ),
                                                'dependency' => array( $key_attr . '_type', '==' , 'product_custom')
                                            ), $parent_size);
                                            echo la_fw_add_element(array(
                                                'id'             => $key_attr . '_layout',
                                                'type'           => 'select',
                                            ...
                                            echo la_fw_add_element(array(
                                                ...
                                                'title'          => esc_html__('Style', 'la-studio'),
                                                'default'        => 'circle',
                                                                ...
                                                                <table class="widefat">
                                                                    <thead>
                                                                        <th class="attribute_swatch_preview">
                                                                            <?php esc_html_e( 'Preview', 'la-studio' ); ?>
                                                                        </th>
                                                                        <th class="attribute_swatch_label">
                                                                            <?php esc_html_e( 'Attribute', 'la-studio' ); ?>
                                                                        </th>
                                                                        <th class="attribute_swatch_type">
                                                                            <?php esc_html_e( 'Type', 'la-studio' ); ?>
                                                                        </th>
                                                                    </thead>
                                                                </table>
                                                            </div>
        ...
        public static function render_attribute_in_product_list_loop( $product ){
            $la_swatches_configs = get_option('la_swatches_configs', array());
            ...
            if(!empty($attribute_allow)){
                $new_instance = self::get_instance();
                add_filter('LaStudio/swatches/configuration_object/get_swatch_display_style', array( $new_instance, 'override_swatch_style_in_list'), 10, 2);
                echo $new_instance->_render_option_by_attribute( $product, $attribute_allow );
                remove_filter('LaStudio/swatches/configuration_object/get_swatch_display_style', array( $new_instance, 'override_swatch_style_in_list'), 10);
            }
        }
        ...
        */
        ...
        public function add_additional_into_variation_json( $variation_data, $wc_product_variable, $variation_obj ) {
            ...
            $_360_enable = get_post_meta( $wc_product_variable->get_id(), '_la_360_enable', true );
            if($_360_enable != 'yes') {
