                ...
                <div id="background-canvas" class="col-12 px-3 py-4 mb-0" style="min-height:90vh; background-color: {{ $data->background_color ?? '#ffffff' }};">
                    ...
                    <div class="row col-12 mr-0 pr-0">
                        ...
                        <div class="col-lg-8 col-md-8 col-sm-8 col-8 pt-3 text-right">
                            <a href="{{url('download_vcard')}}" id="preview-save-contact" style="background-color: {{ $data->save_color ?? '#343A40' }};" class="btn btn-lg btn-dark">SAVE CONTACT</a>
                        </div>
                    </div>
                        ...
                        @foreach($list_contents as $content)
                            @if($content->content_type_id == 1)
                                <div id="div-preview-item-{{$content->id}}" style="display: {{$content->is_content_displayed ? '' : 'none'}}">
                                    <a href="{{url('open_link/'.$content->id)}}" target="_blank" id="preview-item-button-name-{{$content->id}}" data-text-color="{{$content->text_color}}" data-button-color="{{$content->button_color}}" class="btn btn-dark btn-lg col-12 mb-3" style="background-color: {{$content->button_color}}">
