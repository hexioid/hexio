                ...
                <div id="background-canvas" class="col-12 px-0 pb-4 pt-5 mb-0" style="min-height:90vh; ">
                    ...
                    <div class="row col-12 mr-0 pr-0">
                        ...
                        <div class="col-lg-8 col-md-8 col-sm-8 col-8 pt-5 px-0 text-right">
                            <a href="{{url('page/download_vcard').'/'.$data->id}}" id="preview-save-contact" style="background-color: {{ $data->save_color ?? '#262626' }};" class="btn btn-lg btn-dark border-0">SAVE CONTACT</a>
                        </div>
                    </div>
                    <br>
                        ...
                        @foreach($list_contents as $content)
                            @if($content->content_type_id == 1)
                                <div id="div-preview-item-{{$content->id}}" style="display: {{$content->is_content_displayed ? '' : 'none'}}">
                                    <a href="{{url('page/open_link/'.$content->id)}}" target="_blank" id="preview-item-button-name-{{$content->id}}" data-text-color="{{$content->text_color}}" data-button-color="{{$content->button_color}}" class="{{$content->text != null && $content->is_icon_displayed ? 'pl-5' : ''}} btn-preview btn btn-dark btn-lg col-12 mb-3 border-0" style="min-height:50px !important; background-color: {{$content->button_color}}">
